...
gulp.task('default', function() {
    ...
    buildJSMain(SRC_ROOT+'/js/neemHierarchyBarchart.js', 'neemHierarchyBarchart.js', BUILD_ROOT, true,true);
    moveIndexHTML(BUILD_ROOT);
});

gulp.task('debug', function () {
    buildCSSMain(SRC_ROOT, DEBUG_ROOT);
    buildJSMain(SRC_ROOT+'/js/neemHierarchyBarchart.js', 'neemHierarchyBarchart.js', DEBUG_ROOT, true,false);
    moveIndexHTML(DEBUG_ROOT);
});

function buildCSSMain(src, dest){
    gulp.src(src+'/styles/neemHierarchyBarchart.scss')
        .pipe(sass())
